...
export default function IconSelectionModal({ visible, onClose, icons, onSelectIcon }) {
    // Local state for the selected icon
    const [tempSelectedIcon, setTempSelectedIcon] = useState(null);

    const handleSave = () => {
        if (tempSelectedIcon) {
            onSelectIcon(tempSelectedIcon); // Pass selected icon to parent
                    ...
                    renderItem={({ item }) => (
                        <TouchableOpacity onPress={() => setTempSelectedIcon(item)}>
                            <Image
                                ...
                                style={[
                                    styles.iconPreview,
                                    tempSelectedIcon === item && styles.selectedIconStyle, // Highlight selected icon
                                ]}
                            />
                        </TouchableOpacity>
                    )}
...
const styles = StyleSheet.create({
    ...
    selectedIconStyle: {
        borderColor: '#4CAF50', // Highlight border for selected icon
    },
    buttonContainer: {
        flexDirection: 'row',
